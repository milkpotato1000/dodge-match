...
![Neon Orbit Arcade 금지색 오입력 한 칸 재추첨과 개별 타이머 가로 인게임 콘셉트](../design/concepts/neon-orbit-forbidden-zone-timer-v16.png)

이 시안은 `02:23.456`에 파란 물결 원을 잘못 눌러 `Miss(FORBIDDEN)`이 확정된 직후를 보여준다. 원이 잠길 때 강아지가 있던 우하단 칸만 파랑 `≈`에서 보라 `◐`로 즉시 재추첨되고, 나머지 빨강 `│`·노랑 `✦`·초록 `◆` 칸은 그대로다. 우하단 타이머만 `10.000`으로 다시 시작하며 다른 세 칸은 기존 `06.544`를 유지한다. 강아지는 같은 작은 크기와 고정 충돌 범위로 `ㅠoㅠ` 울음 상태가 되고, 현재 칸이 바뀌었으므로 HUD 금지색도 보라 초승달로 갱신된다. 눌린 파란 원은 변경 전 상태로 이미 잠겼기 때문에 `MISS / FORBIDDEN` 피드백을 파란색으로 끝까지 유지한다. 약 26개의 중성 야구공, Match 색별 바닥 각인, 0.125U 헤어라인 경계와 깨끗한 단색 바닥 규칙은 그대로다.

오입력 전 기본 바닥 [v15](../design/concepts/neon-orbit-match-glyph-floor-v15.png), 문양이 없는 야구공 시안 [v14](../design/concepts/neon-orbit-baseball-hazards-v14.png), 점 위험물을 사용한 [v13](../design/concepts/neon-orbit-orbital-floor-v13.png), 완전 단색 바닥의 [v12](../design/concepts/neon-orbit-hairline-boundary-v12.png), 0.5U 경계의 [v11](../design/concepts/neon-orbit-minimal-zone-margin-v11.png), 1U 경계와 이전 Dodge 문양이 남은 [v10](../design/concepts/neon-orbit-thin-zone-margin-v10.png), 넓은 2U 경계의 [v9](../design/concepts/neon-orbit-small-dog-fixed-hitbox-v9.png), 이전 크기의 [v8](../design/concepts/neon-orbit-raised-difficulty-v8.png)과 정상 상태 [v6](../design/concepts/neon-orbit-gameplay-neutral-dense-dots-v6.png)는 변화 기록으로 보존한다.
...
[한 칸 재추첨·개별 타이머·우는 강아지와 Match Miss 동시 표현 전체 화면 v16](../design/concepts/neon-orbit-forbidden-zone-timer-v16.png)
...
- 페널티 색 전환은 긴 페이드 없이 한 프레임에 교체한다. 바뀐 칸이 캐릭터의 현재 칸이면 조명·위 배지·HUD 금지색 칩도 같은 프레임에 갱신한다. 해당 칸의 타이머만 10초로 다시 시작하고 다른 칸의 카운트다운은 그대로 이어진다.
- 첫 Miss 예시에서는 점수 콤보가 `×0`으로 초기화되고 Match 생명은 `60→52`, `−8`로 표시된다. 실제 감소량은 현재 Miss 연속 횟수 규칙을 따른다.
- v16은 우하단 한 칸만 파랑에서 보라로 바뀌고 타이머가 `10.000`으로 분리되는 순간을 보여준다. 다른 세 칸은 색과 `06.544` 타이머를 유지하고, 야구공과 궤적은 아이보리·남청·흰색·회색만 사용한다.
...
- 색 전환 시간: 모든 칸의 다음 변경 시각이 같으면 `SHIFT ALL 03.742`처럼 소수점 세 자리까지 표시한다. 하나라도 분리되면 현재 구역 구조와 같은 1·2·3·4칸 미니 배열에 각 칸의 문양과 `03.742`를 표시하고 캐릭터가 서 있는 칸의 타이머 외곽선을 밝힌다. 차트의 동기 전환 마지막 0.500초에만 `LOCK`을 표시한다.
...
- 차트의 동기 전환 3초 전부터 연결 HUD의 `SHIFT ALL`이 강조된다. 0.500초 전에는 플레이필드 외곽이 안쪽으로 한 번 수축해 생성 중지 구간을 알린다. 분리된 칸은 자기 타이머만 강조하고 원 생성 중지 연출은 사용하지 않는다.
...
- `Miss(FORBIDDEN)` 페널티 전환은 3초 예고·`LOCK` 표시·0.5초 생성 중지 없이 즉시 발생한다. 잠긴 원에 저장된 칸 하나와 그 바닥 문양만 한 프레임에 교체하고 해당 칸의 타이머를 10초로 재설정한다. 다른 칸 타이머는 건드리지 않는다.
