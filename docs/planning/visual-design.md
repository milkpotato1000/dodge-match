...
![Neon Orbit Arcade 시드 초기조건과 물리 반사를 적용한 Dodge 점, 크게 축소한 강아지 가로 인게임 콘셉트](../design/concepts/neon-orbit-small-dog-fixed-hitbox-v9.png)

이 시안은 확정된 가로 16:9, 좌우 50:50 구조를 반영한다. `02:23.456` 시점의 중후반 플레이를 가정해 Dodge의 4색 구역과 약 26개의 중성색 점, 표시 반지름 2U로 크게 줄인 반려견 캐릭터, Match 안전 영역 안에 무작위로 흩어진 동시 원 4개를 표현했다. 파란 금지색 원을 잘못 누른 순간이라 Match에는 `MISS / FORBIDDEN`, Dodge에는 `ㅠoㅠ` 울음 포즈가 동시에 보인다. Dodge 점의 몸체·중심·이동 꼬리에는 색을 사용하지 않는다. 점은 생성 위치와 최초 진행각만 판의 시드로 추첨하고, 활성화 뒤에는 입사각과 반사각이 같은 물리 반사로만 움직인다. Dodge 구역의 검은 십자 경계는 2U 판정 마진을 나타내며 중앙 교차점에는 원·`+`·아이콘을 두지 않는다. Match 영역에는 격자·슬롯·사분면이 없으며 원 위치도 고정 좌표가 아닌 한 번의 시드 기반 무작위 배치 예시다. 점 개수는 게임플레이 문서의 `3/6/10/15/22/28/32개` 곡선을 따른다.

이전 크기의 [v8](../design/concepts/neon-orbit-raised-difficulty-v8.png)과 정상 상태 [v6](../design/concepts/neon-orbit-gameplay-neutral-dense-dots-v6.png)는 변화 기록으로 보존한다. 폐기한 세로 구도 [v1](../design/concepts/neon-orbit-gameplay-v1.png)과 격자처럼 보이는 Match 배치 [v2](../design/concepts/neon-orbit-gameplay-landscape-v2.png)는 디자인 기준으로 사용하지 않는다.
...
[시드 초기조건·물리 반사 Dodge 궤적과 크게 축소된 강아지, Match Miss 동시 표현 전체 화면 v9](../design/concepts/neon-orbit-small-dog-fixed-hitbox-v9.png)
...
- v9은 `02:23.456`의 약 26개 중후반 점 밀도와 2U로 크게 줄인 강아지 표시 크기를 함께 확인하는 시안이다. 점과 궤적은 흰색·회색만 사용한다.
