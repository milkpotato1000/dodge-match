...
![Neon Orbit Arcade 상향 난이도와 축소 강아지 가로 인게임 콘셉트](../design/concepts/neon-orbit-raised-difficulty-v8.png)

이 시안은 확정된 가로 16:9, 좌우 50:50 구조를 반영한다. `02:23.456` 시점의 중후반 플레이를 가정해 Dodge의 4색 구역과 약 26개의 중성색 점, 2.7U로 줄인 반려견 캐릭터, Match 안전 영역 안에 무작위로 흩어진 동시 원 4개를 표현했다. 파란 금지색 원을 잘못 누른 순간이라 Match에는 `MISS / FORBIDDEN`, Dodge에는 `ㅠoㅠ` 울음 포즈가 동시에 보인다. Dodge 점의 몸체·중심·이동 꼬리에는 색을 사용하지 않는다. Dodge 구역의 검은 십자 경계는 2U 판정 마진을 나타내며 중앙 교차점에는 원·`+`·아이콘을 두지 않는다. Match 영역에는 격자·슬롯·사분면이 없으며 원 위치도 고정 좌표가 아닌 한 번의 시드 기반 무작위 배치 예시다. 점 개수는 게임플레이 문서의 `3/6/10/15/22/28/32개` 곡선을 따른다.

이전 정상 상태의 [v6](../design/concepts/neon-orbit-gameplay-neutral-dense-dots-v6.png)는 변화 기록으로 보존한다. 폐기한 세로 구도 [v1](../design/concepts/neon-orbit-gameplay-v1.png)과 격자처럼 보이는 Match 배치 [v2](../design/concepts/neon-orbit-gameplay-landscape-v2.png)는 디자인 기준으로 사용하지 않는다.
...
- 마스터 자산은 충분히 큰 투명 PNG로 보관하고 실제 게임에서는 약 26~30dp로 표시한다. 축소했을 때 귀·얼굴 윤곽과 핵심 무늬가 남아야 한다.
- 충돌 판정은 그림의 털이나 귀 끝과 분리한다. 표시 반지름 2.7U 안에 캐릭터를 배치하고 실제 원형 충돌 반지름은 2.25U로 고정한다. 이전 3U 안보다 그림만 약 10% 줄여 고밀도 점 사이의 경로를 더 잘 보이게 한다.
...
[상향된 Dodge 밀도와 축소된 강아지, Match Miss 동시 표현 전체 화면 v8](../design/concepts/neon-orbit-raised-difficulty-v8.png)
...
- v8은 `02:23.456`의 약 26개 중후반 점 밀도와 2.7U로 줄인 강아지 표시 크기를 함께 확인하는 시안이다. 점과 궤적은 흰색·회색만 사용한다.
...
- 반응 중에도 Dodge 이동, 점 충돌, Match 차트와 게임 시계는 계속된다. 표시 자산만 바뀌며 2.7U 표시 범위와 2.25U 충돌 반지름은 변하지 않는다.
