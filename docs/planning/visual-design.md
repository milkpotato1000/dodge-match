...
![Neon Orbit Arcade 금지색 오입력 한 칸 재추첨과 5.300초 개별 타이머 가로 인게임 콘셉트](../design/concepts/neon-orbit-forbidden-zone-timer-v17.png)

이 시안은 `02:23.456`에 파란 물결 원을 잘못 눌러 `Miss(FORBIDDEN)`이 확정된 직후를 보여준다. 원이 잠길 때 강아지가 있던 우하단 칸만 파랑 `≈`에서 보라 `◐`로 재추첨되고, 나머지 빨강 `│`·노랑 `✦`·초록 `◆` 칸은 그대로다. 우하단 타이머만 `5.300`으로 다시 시작하며 다른 세 칸은 기존 `02.744`를 유지한다. 논리 색과 HUD는 즉시 보라로 바뀌고 바닥은 이전 색에서 새 색까지 1초 동안 페이드한다. 강아지는 같은 작은 크기와 고정 충돌 범위로 `ㅠoㅠ` 울음 상태가 된다. 눌린 파란 원은 변경 전 상태로 이미 잠겼기 때문에 `MISS / FORBIDDEN` 피드백을 파란색으로 끝까지 유지한다. 약 26개의 중성 야구공, Match 색별 바닥 각인, 0.125U 헤어라인 경계와 깨끗한 단색 바닥 규칙은 그대로다.

10초 타이머의 [v16](../design/concepts/neon-orbit-forbidden-zone-timer-v16.png), 오입력 전 기본 바닥 [v15](../design/concepts/neon-orbit-match-glyph-floor-v15.png), 문양이 없는 야구공 시안 [v14](../design/concepts/neon-orbit-baseball-hazards-v14.png), 점 위험물을 사용한 [v13](../design/concepts/neon-orbit-orbital-floor-v13.png), 완전 단색 바닥의 [v12](../design/concepts/neon-orbit-hairline-boundary-v12.png), 0.5U 경계의 [v11](../design/concepts/neon-orbit-minimal-zone-margin-v11.png), 1U 경계와 이전 Dodge 문양이 남은 [v10](../design/concepts/neon-orbit-thin-zone-margin-v10.png), 넓은 2U 경계의 [v9](../design/concepts/neon-orbit-small-dog-fixed-hitbox-v9.png), 이전 크기의 [v8](../design/concepts/neon-orbit-raised-difficulty-v8.png)과 정상 상태 [v6](../design/concepts/neon-orbit-gameplay-neutral-dense-dots-v6.png)는 변화 기록으로 보존한다.
...
```text
┌────────────────────────────────────────────────────────────────────┐
│ SCORE 012,480   02:23.456   ×18   LIFE ███████░   SHIFT ALL 3.742│ HUD 9%
├─────────────────────────────────┬──────────────────────────────────┤
│                                 │                                  │
│    점 •     Pip       •         │       ◯          ◯               │
│                                 │            ◯                     │
...
[한 칸 재추첨·5.300초 개별 타이머·우는 강아지와 Match Miss 동시 표현 전체 화면 v17](../design/concepts/neon-orbit-forbidden-zone-timer-v17.png)
...
- Dodge에서는 강아지가 `ㅠoㅠ` 울음 포즈로 480ms 바뀌는 동시에 해당 원을 금지로 만든 칸 하나의 바닥색과 Match 문양만 즉시 바뀐다. 강아지 옆에는 `MISS` 문자를 중복 표시하지 않는다.
- Match 쪽에는 눈물이나 강아지 감정 아이콘을 표시하지 않는다. 플레이어가 시선을 어느 영역에 두고 있어도 같은 실수를 인지하되 의미는 중복하지 않는다.
- 페널티 판정 스텝에 논리 색을 교체하고, 바닥색과 문양은 이전 화면에서 새 색까지 1초 동안 교차 페이드한다. 바뀐 칸이 캐릭터의 현재 칸이면 조명·위 배지·HUD 금지색 칩도 같은 스텝에 갱신한다. 해당 칸의 타이머만 `5.300`으로 다시 시작하고 다른 칸의 카운트다운은 그대로 이어진다.
- 4색 화면은 `45.000초`에 시작하고 네 칸의 첫 정기 전환은 모두 `50.300초`다. 실수 전에는 네 타이머가 함께 움직이며, 금지색 오입력 뒤에는 해당 칸만 입력 시각 기준의 5.300초 주기로 분리된다. 이 비동기화는 한 번의 Miss가 남은 플레이 내내 추적 대상을 늘리는 큰 시각적 페널티다.
- 첫 Miss 예시에서는 점수 콤보가 `×0`으로 초기화되고 Match 생명은 `60→52`, `−8`로 표시된다. 실제 감소량은 현재 Miss 연속 횟수 규칙을 따른다.
- v17은 우하단 한 칸만 파랑에서 보라로 바뀌고 타이머가 `5.300`으로 분리되는 순간을 보여준다. 다른 세 칸은 색과 `02.744` 타이머를 유지하고, 야구공과 궤적은 아이보리·남청·흰색·회색만 사용한다.
...
- 금지색 칩: `DON'T TAP` 라벨, 신호색 선, 고유 기호, 색 이름을 함께 표시한다. 바닥 판정점이 헤어라인 반대편 가장자리를 벗어나거나 현재 칸이 페널티로 재추첨된 스텝에 캐릭터 조명·위 배지와 상태값을 함께 바꾸고 100ms 동안 표시만 교차 전환한다.
- Match 생명: 길이 140dp 이상 수평 게이지. 현재 수치 `60`을 내부에 표시한다. 회복은 오른쪽으로 흐르는 광선과 `+4`, 중립은 짧은 흰색 테두리, 감소는 왼쪽으로 밀리는 절단선과 `−8`로 표현한다.
- 색 전환 시간: 모든 칸의 다음 변경 시각이 같으면 `SHIFT ALL 03.742`처럼 소수점 세 자리까지 표시한다. 하나라도 분리되면 현재 구역 구조와 같은 1·2·3·4칸 미니 배열에 각 칸의 문양과 `03.742`를 표시하고 캐릭터가 서 있는 칸의 타이머 외곽선을 밝힌다. 전환 직전에도 원이 계속 등장하므로 `LOCK` 문구는 사용하지 않는다.
...
- 차트의 동기 전환 3초 전부터 연결 HUD의 `SHIFT ALL`이 강조된다. 원 생성은 중지하지 않으며 플레이필드 외곽 수축이나 `LOCK` 표시는 사용하지 않는다. 분리된 칸은 자기 타이머만 강조한다.
- 4색 단계 이후 정기 재추첨은 T−0.500초부터 기존 색·문양을 페이드아웃하고 T에서 논리 색을 교체한 뒤 T+0.500초까지 새 색·문양을 페이드인한다. 총 길이는 1초다.
- T 직후 120ms 동안 구역마다 중심에서 바깥으로 얇은 확인선이 퍼진다. 이 효과는 색 적용 이후의 확인 피드백이다.
- 캐릭터의 금지색 배지와 연결 HUD는 같은 프레임에 새 색·기호로 바뀐다.
- `Miss(FORBIDDEN)` 페널티 전환은 3초 예고 없이 즉시 논리 색을 바꾸고, 잠긴 원에 저장된 칸 하나만 1초 교차 페이드한다. 해당 칸의 타이머는 `5.300`으로 재설정하며 다른 칸 타이머와 원 생성은 건드리지 않는다.
...
8. 결과 점수판
9. 로컬 리더보드와 이름 입력
10. 설정: 좌우 배치, PC 이동 키 `WASD / 방향키`, 음악 켜기/끄기, 음악·효과음 음량, 진동, Reduced Effects, 색·기호 안내
...
디자인 단계에서 이벤트 이름만 고정하고 실제 사운드는 별도 제작한다: `target_spawn`, `ring_contact`, `perfect`, `great`, `good`, `miss`, `forbidden_tap`, `forbidden_zone_reroll`, `life_gain`, `life_loss`, `zone_shift`, `dodge_bounce`, `dodge_hit`, `run_clear`. 모든 이벤트는 시각 표현을 먼저 갖는다.
...
- `02:23.456`과 `SHIFT ALL 03.742` 또는 칸별 타이머를 혼동하지 않도록 라벨·미니맵 위치와 숫자 폭이 고정되어 있는가.
