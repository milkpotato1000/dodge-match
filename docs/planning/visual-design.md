...
캐릭터 가칭은 **Pip**으로 둔다. 작은 둥근 마름모 몸체와 큰 눈 하나, 이동 방향으로 짧게 기울어지는 두 개의 핀으로 구성한다. 특정 성별·종족을 정하지 않아 작은 아이콘 크기에서도 인식되고 스킨 확장이 쉽다.

- 권장 표시 크기: 30~34dp. 실제 원형 충돌 반지름은 몸체 외곽의 75%라는 현재 기획 제안을 따른다.
- 몸체: `text-primary` 흰색, 3dp `shadow` 외곽선. 어느 색 구역 위에서도 형태가 유지된다.
- 중심 눈: 이동 반대쪽이 아니라 이동 방향을 1~2dp 바라본다. 입력 반응을 전달하되 충돌 판정에는 영향을 주지 않는다.
- 현재 구역색: 몸체 아래에 신호색 그림자와 고유 기호 배지를 붙인다. 배지는 캐릭터와 함께 움직여 금지색 변화를 즉시 보여준다.
- 드래그 중: 핀이 이동 방향으로 12° 기울고 80ms 길이의 짧은 잔상이 생긴다. 잔상은 판정 크기로 보이지 않도록 25% 이하 불투명도를 쓴다.
- 정지 중: 1.2초 주기의 2% 호흡 모션만 사용한다. 위치는 움직이지 않는다.
- 위험 근접: 점이 캐릭터 외곽의 경고 반경에 들어오면 외곽선이 1회 수축한다. 지속 점멸은 피한다.

표정은 평상시의 집중 눈, 판정 연속 성공 시의 반짝임, 충돌 시의 나선 눈 세 상태만 우선 제작한다. 감정 애니메이션이 점의 위치를 가리지 않게 얼굴 내부에서 끝낸다.
...
- 금지색 칩: `DON'T TAP` 라벨, 신호색 선, 고유 기호, 색 이름을 함께 표시한다. 캐릭터가 경계를 넘으면 100ms 교차 전환하며 위치 판정은 즉시 갱신한다.
...
- 평상시 구역 경계는 2dp의 어두운 선으로 고정한다. 경계선 자체가 움직이지 않게 해 캐릭터 위치 판단을 돕는다.
...
7. 결과 점수판
8. 로컬 리더보드와 이름 입력
9. 설정: 좌우 배치, 음량, 진동, Reduced Effects, 색·기호 안내

### 컴포넌트와 상태

- 캐릭터: idle, drag, near-hit, hit, success 반응
...
- 캐릭터 Pip은 최종 캐릭터 제작 전까지 사용할 플레이스홀더 방향이다.
