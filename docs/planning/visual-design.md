...
![Neon Orbit Arcade 무작위 타깃 가로 인게임 콘셉트](../design/concepts/neon-orbit-gameplay-random-targets-v5.png)

이 시안은 확정된 가로 16:9, 좌우 50:50 구조를 반영한다. `01:43.456` 시점의 중후반 플레이를 가정해 Dodge의 4색 구역과 증가한 점 밀도, 파란 구역 위의 반려견 캐릭터, Match 안전 영역 안에 무작위로 흩어진 동시 원 4개, 파란색 금지 상태의 캐릭터 조명·HUD 칩·원 보호띠 연결을 한 화면에 표현했다. Dodge 구역의 십자 경계는 색 판정 마진만 나타내며 중앙 교차점에는 원·`+`·아이콘을 두지 않는다. Match 영역에는 격자·슬롯·사분면이 없으며 시안의 위치도 고정 좌표가 아닌 한 번의 시드 기반 무작위 배치 예시다. 원과 점의 정확한 크기·개수·배치는 구현 QA에서 논리 단위와 터치 영역 기준으로 조정한다.
