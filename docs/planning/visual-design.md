...
![Neon Orbit Arcade 야구공 위험물과 중성 궤도선 바닥 가로 인게임 콘셉트](../design/concepts/neon-orbit-baseball-hazards-v14.png)

이 시안은 확정된 가로 16:9, 좌우 50:50 구조를 반영한다. `02:23.456` 시점의 중후반 플레이를 가정해 Dodge의 4색 구역과 약 26개의 중성색 야구공, 표시 반지름 2U로 줄인 반려견 캐릭터, Match 안전 영역 안에 무작위로 흩어진 동시 원 4개를 표현했다. Dodge 구역 바닥의 반복 문양과 큰 기호, 플레이필드의 `DODGE / MATCH` 제목은 제거했다. 빈 느낌은 Match 기호 대신 네 구역 전체를 가로지르는 2~4% 명도의 거대한 중성 궤도선과 넓은 비네트로 보완한다. 검은 십자 경계는 총 0.125U의 1px급 헤어라인이며, 강아지 아래 청백색 점이 실제 구역 판정에 쓰는 공통 바닥 피벗을 보여준다. 야구공은 생성 위치와 최초 진행각만 판의 시드로 추첨하고 활성화 뒤에는 정반사로만 움직인다. Match 원 위치는 시드 기반 무작위 배치 예시이며, 야구공 개수는 게임플레이 문서의 `3/6/10/15/22/28/32개` 곡선을 따른다.

점 위험물을 사용한 [v13](../design/concepts/neon-orbit-orbital-floor-v13.png), 완전 단색 바닥의 [v12](../design/concepts/neon-orbit-hairline-boundary-v12.png), 0.5U 경계의 [v11](../design/concepts/neon-orbit-minimal-zone-margin-v11.png), 1U 경계와 Dodge 문양이 남은 [v10](../design/concepts/neon-orbit-thin-zone-margin-v10.png), 넓은 2U 경계의 [v9](../design/concepts/neon-orbit-small-dog-fixed-hitbox-v9.png), 이전 크기의 [v8](../design/concepts/neon-orbit-raised-difficulty-v8.png)과 정상 상태 [v6](../design/concepts/neon-orbit-gameplay-neutral-dense-dots-v6.png)는 변화 기록으로 보존한다.
...
2. **색 이외의 단서:** 7색의 고유 기호는 Match 원, 캐릭터 위 현재색 배지와 HUD 칩에만 표시한다. Dodge 바닥은 단색으로 유지해 야구공의 궤적을 가리지 않는다.
...
- 네 색 구역 전체 뒤에 동일한 중성 환경 레이어 하나를 둔다. 구역마다 다른 장식을 배치하지 않는다.
- 환경 레이어는 화면보다 큰 열린 궤도 곡선 2~3개와 넓은 중심광·가장자리 비네트만 사용한다.
- 색은 `ink-950` 또는 채도를 뺀 슬레이트이며 체감 명도 차이는 2~4%로 제한한다. 구역색을 나타내거나 바꾸지 않는다.
- 완성된 원, 별, 삼각형, 물결처럼 Match 기호로 읽히는 형태는 만들지 않는다. 작은 점, 입자, 격자와 짧은 선도 야구공이나 이동 꼬리와 혼동되므로 쓰지 않는다.
...
[중성 야구공·궤도선 바닥·0.125U 경계와 작은 강아지, Match Miss 동시 표현 전체 화면 v14](../design/concepts/neon-orbit-baseball-hazards-v14.png)
...
- v14는 `02:23.456`의 약 26개 중후반 야구공 밀도, 2U 강아지 표시 크기, 0.125U 헤어라인 경계와 2~4% 중성 궤도선 바닥을 함께 확인하는 시안이다. 야구공과 궤적은 아이보리·남청·흰색·회색만 사용한다.
...
- Dodge 바닥은 구역별 문양 없이 2~4% 중성 대형 궤도선과 넓은 명암만 사용하고 플레이필드의 `DODGE / MATCH` 제목을 표시하지 않는다. 색 기호는 Match 원, 캐릭터 위 배지와 HUD 칩에만 사용한다.
