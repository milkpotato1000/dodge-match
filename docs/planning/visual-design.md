...
[0.125U 헤어라인 경계·명확한 판정점과 작은 강아지, Match Miss 동시 표현 전체 화면 v12](../design/concepts/neon-orbit-hairline-boundary-v12.png)
...
- v12는 `02:23.456`의 약 26개 중후반 점 밀도, 2U 강아지 표시 크기, 0.125U 헤어라인 경계와 청백색 바닥 판정점을 함께 확인하는 시안이다. 점과 궤적은 흰색·회색만 사용한다.
...
1. 타이틀·플레이 시작
2. 첫 플레이 튜토리얼: 방향 버튼 이동, 허용색 터치, 금지색 미터치
3. 3초 카운트다운
4. 인게임 기본·고밀도 상태
5. 일시정지·백그라운드 복귀 안내
6. 원인별 게임 오버와 곡 완주
...
- D-pad와 엄지가 하단 모서리를 가린 상태에서도 캐릭터와 위험 점을 추적할 수 있는가. 버튼 바탕은 반투명으로 두고 점·예고·이동 꼬리는 버튼 그래픽보다 위 레이어에 그린다.
