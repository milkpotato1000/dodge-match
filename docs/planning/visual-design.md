...
![Neon Orbit Arcade 최소 구역 경계와 무문양 Dodge 바탕 가로 인게임 콘셉트](../design/concepts/neon-orbit-minimal-zone-margin-v11.png)

이 시안은 확정된 가로 16:9, 좌우 50:50 구조를 반영한다. `02:23.456` 시점의 중후반 플레이를 가정해 Dodge의 4색 단색 구역과 약 26개의 중성색 점, 표시 반지름 2U로 줄인 반려견 캐릭터, Match 안전 영역 안에 무작위로 흩어진 동시 원 4개를 표현했다. 파란 금지색 원을 잘못 누른 순간이라 Match에는 `MISS / FORBIDDEN`, Dodge에는 `ㅠoㅠ` 울음 포즈가 동시에 보인다. Dodge 구역 바닥의 반복 문양과 큰 기호, 플레이필드의 `DODGE / MATCH` 제목은 모두 제거했다. 고유 기호는 Match 원 내부, 강아지 바로 위 현재색 배지와 공통 HUD 칩에만 남긴다. 점은 생성 위치와 최초 진행각만 판의 시드로 추첨하고, 활성화 뒤에는 입사각과 반사각이 같은 물리 반사로만 움직인다. Dodge 구역의 검은 십자 경계는 총 0.5U 판정 마진을 나타내며 중앙 교차점에는 장식을 두지 않는다. Match 원 위치는 고정 좌표가 아닌 한 번의 시드 기반 무작위 배치 예시다. 점 개수는 게임플레이 문서의 `3/6/10/15/22/28/32개` 곡선을 따른다.

1U 경계와 Dodge 문양이 남은 [v10](../design/concepts/neon-orbit-thin-zone-margin-v10.png), 넓은 2U 경계의 [v9](../design/concepts/neon-orbit-small-dog-fixed-hitbox-v9.png), 이전 크기의 [v8](../design/concepts/neon-orbit-raised-difficulty-v8.png)과 정상 상태 [v6](../design/concepts/neon-orbit-gameplay-neutral-dense-dots-v6.png)는 변화 기록으로 보존한다. 폐기한 세로 구도 [v1](../design/concepts/neon-orbit-gameplay-v1.png)과 격자처럼 보이는 Match 배치 [v2](../design/concepts/neon-orbit-gameplay-landscape-v2.png)는 디자인 기준으로 사용하지 않는다.
...
2. **색 이외의 단서:** 7색의 고유 기호는 Match 원, 캐릭터 위 현재색 배지와 HUD 칩에만 표시한다. Dodge 바닥은 단색으로 유지해 위험 점의 궤적을 가리지 않는다.
...
- Dodge 구역은 문양·반복 패턴·큰 모서리 기호가 없는 단색 또는 약한 명도 그라디언트로 채운다.
...
- 현재 구역색은 캐릭터 아래의 반투명 원형 조명과 캐릭터 바로 위의 고유 기호 배지로 표시한다. 이 효과는 캐릭터 고유 털색을 덮지 않는다.
...
[0.5U 구역 경계·무문양 Dodge 바탕과 작은 강아지, Match Miss 동시 표현 전체 화면 v11](../design/concepts/neon-orbit-minimal-zone-margin-v11.png)
...
- v11은 `02:23.456`의 약 26개 중후반 점 밀도, 2U 강아지 표시 크기, 0.5U 구역 경계와 무문양 단색 바탕을 함께 확인하는 시안이다. 점과 궤적은 흰색·회색만 사용한다.
...
- 반응 중에도 Dodge 이동, 점 충돌, Match 차트와 게임 시계는 계속된다. 표시 자산만 바뀌며 2U 표시 범위와 1.5U 충돌 반지름은 변하지 않는다.
...
점은 네 색 구역을 합친 Dodge 전체 사각형을 한 공간으로 사용한다. 내부 십자 경계를 그대로 통과하며 최외곽 벽에서만 반사한다. 인게임 시안에서는 일부 점의 이동 꼬리가 색 경계를 가로지르게 배치해 구역별로 갇혀 움직인다는 오해를 막고, 내부 경계에는 반사광이나 충돌 효과를 표시하지 않는다.
...
- 평상시 구역 경계는 총 폭 0.5U, 중심선 양쪽 0.25U의 어두운 십자 마진으로 고정한다. 경계선 자체가 움직이지 않게 해 캐릭터 위치 판단을 돕는다. 십자 교차점에도 별도의 원, `+` 또는 장식 아이콘을 두지 않는다.
- 캐릭터가 경계 마진에 들어오면 현재 판정 구역의 단색 바탕을 캐릭터 주변에서 조금 더 밝히고, 반투명 원형 조명을 현재 판정색으로 유지한다.
...
- Dodge 바닥은 무문양 단색으로 유지하고 플레이필드의 `DODGE / MATCH` 제목을 표시하지 않는다. 색 기호는 Match 원, 캐릭터 위 배지와 HUD 칩에만 사용한다.
