...
![Neon Orbit Arcade 가로 인게임 콘셉트](../design/concepts/neon-orbit-gameplay-landscape-v2.png)

이 시안은 확정된 가로 16:9, 좌우 50:50 구조를 반영한다. `01:43.456` 시점의 중후반 플레이를 가정해 Dodge의 4색 구역과 증가한 점 밀도, Match의 동시 원 4개, 파란색 금지 상태의 캐릭터 조명·HUD 칩·원 보호띠 연결을 한 화면에 표현했다. 원과 점의 정확한 크기·개수·배치는 구현 QA에서 논리 단위와 터치 영역 기준으로 조정한다.

폐기한 세로 구도는 시각 변화 기록을 위해 [v1 이미지](../design/concepts/neon-orbit-gameplay-v1.png)로만 보존하며 디자인 기준으로 사용하지 않는다.
