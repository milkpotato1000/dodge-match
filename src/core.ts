export const COLORS = [
  0xf45d76, 0xffad66, 0xf8da72, 0x65dba6, 0x65b9ff, 0x929aff, 0xcd91ef,
];
export const GLYPHS = ["│", "▲", "✦", "◆", "≈", ":", "◐"];
...
export class Score {
  ...
  judge(g: Grade, matched = false) {
    ...
    if (g === "Miss") {
      ...
      this.recovery = 0;
      this.missStreak++;
      this.life -= Math.min(20, 4 + 4 * this.missStreak);
    } else {
      this.missStreak = 0;
      this.combo++;
      this.match +=
          ...
          this.recovery >= 20
            ...
            : this.recovery >= 10
              ...
              : this.recovery >= 5
                ? 3
                : 2;
        this.life += amount * (g === "Perfect" ? 2 : 1);
      }
    }
    this.life = Math.max(0, Math.min(100, this.life));
    this.maxCombo = Math.max(this.maxCombo, this.combo);
    this.maxRecovery = Math.max(this.maxRecovery, this.recovery);
    this.maxMiss = Math.max(this.maxMiss, this.missStreak);
  }
  total(time: number) {
    return Math.floor((time + 1e-7) / 1000) * 100 + this.match;
  }
}
export function validateChart(c: Chart) {
  ...
  for (const t of c.targets) {
    if (
      t.at <= last ||
      t.at % 125 !== 0 ||
      t.spawn !== t.at - t.approach ||
      t.spawn < 0 ||
      t.at + 200 > c.durationMs
    )
...
export function direction(x: number, y: number): Vec {
  const d = Math.hypot(x, y);
  return d ? { x: x / d, y: y / d } : { x: 0, y: 0 };
}
export function swept(a: Vec, b: Vec, c: Vec, d: Vec, r: number) {
  const x = a.x - c.x,
    y = a.y - c.y,
    vx = b.x - a.x - d.x + c.x,
    vy = b.y - a.y - d.y + c.y;
  const len = vx * vx + vy * vy;
  const t = len ? Math.max(0, Math.min(1, -(x * vx + y * vy) / len)) : 0;
  return (x + vx * t) ** 2 + (y + vy * t) ** 2 <= r * r + 1e-9;
}
export type Ball = Vec & {
  id: number;
  vx: number;
  vy: number;
  activeAt: number;
...
export function travel(
  ...
  const segments: { a: Vec; b: Vec; start: number; end: number }[] = [];
  let remaining = seconds,
    elapsed = 0;
  while (remaining > 1e-10) {
    const tx =
      ball.vx > 0
        ? (width - BALL_RADIUS - ball.x) / ball.vx
        : ball.vx < 0
          ? (BALL_RADIUS - ball.x) / ball.vx
          : Infinity;
    const ty =
      ball.vy > 0
        ? (height - BALL_RADIUS - ball.y) / ball.vy
        : ball.vy < 0
          ? (BALL_RADIUS - ball.y) / ball.vy
          : Infinity;
    const dt = Math.max(0, Math.min(remaining, tx, ty));
    ...
    segments.push({
      ...
      end: (elapsed + dt) / seconds,
    });
    remaining -= dt;
    elapsed += dt;
    if (tx <= dt + 1e-10) ball.vx = -ball.vx;
    if (ty <= dt + 1e-10) ball.vy = -ball.vy;
...
export class Engine {
  ...
  constructor(
    ...
    validateChart(chart);
    this.ballRng = new Random(seed ^ 0x1234);
    this.matchRng = new Random(seed ^ 0x2345);
  ...
  difficulty() {
    const curve = this.chart.dodgeCurve;
    for (let i = 1; i < curve.length; i++) {
      const [t, c, s] = curve[i],
        [pt, pc, ps] = curve[i - 1];
      if (this.time <= t) {
        const f = (this.time - pt) / (t - pt);
        return {
          count: Math.floor(pc + (c - pc) * f + 1e-8),
          speed: ps + (s - ps) * f,
        };
      }
    }
  ...
  private addBall(initial = false) {
    ...
    for (let attempt = 0; attempt < 500; attempt++) {
      ...
      if (initial) {
        x = BALL_RADIUS + r.next() * (WIDTH - 2 * BALL_RADIUS);
        y = BALL_RADIUS + r.next() * (HEIGHT - 2 * BALL_RADIUS);
        angle = r.next() * Math.PI * 2;
      } else {
        const edge = r.int(4),
          pos = 4 + r.next() * 92,
          a = (((20 + r.next() * 50) * Math.PI) / 180) * (r.int(2) ? 1 : -1);
        x = edge === 0 ? BALL_RADIUS : edge === 1 ? WIDTH - BALL_RADIUS : pos;
        y = edge === 2 ? BALL_RADIUS : edge === 3 ? HEIGHT - BALL_RADIUS : pos;
        angle =
          (edge === 0
            ? 0
            : edge === 1
      ...
      if (
        Math.hypot(x - this.player.x, y - this.player.y) < 15 ||
        this.balls.some((b) => Math.hypot(x - b.x, y - b.y) < BALL_RADIUS * 2)
      )
        continue;
      ...
      this.balls.push({
        ...
        activeAt: this.time + (initial ? 0 : 500),
      });
      return;
    }
  }
  ...
  private spawn() {
    while (
      this.nextTarget < this.chart.targets.length &&
      this.chart.targets[this.nextTarget].spawn <= this.time + 1e-7
    ) {
      const spec = this.chart.targets[this.nextTarget++];
      let p = { x: 22, y: 22 };
      const occupied = this.targets.filter((t) => !t.judged);
      let found = false;
      for (let i = 0; i < 80; i++) {
        p = {
          x: 19 + this.matchRng.next() * 62,
          y: 19 + this.matchRng.next() * 62,
        };
        if (occupied.every((t) => Math.hypot(t.x - p.x, t.y - p.y) >= 27)) {
          found = true;
          break;
        }
      }
      if (!found) {
        ...
        p = candidates.sort(
          (a, b) =>
            Math.min(...occupied.map((t) => Math.hypot(t.x - b.x, t.y - b.y))) -
            Math.min(...occupied.map((t) => Math.hypot(t.x - a.x, t.y - a.y))),
        )[0];
      }
      this.targets.push({
        ...spec,
  ...
  step() {
    ...
    for (const b of this.balls) {
      if (b.activeAt > this.time) continue;
      if (
        b.activeAt > this.time - STEP &&
        b.activeAt > 0 &&
        Math.hypot(b.x - this.player.x, b.y - this.player.y) < 15
      ) {
        b.activeAt = this.time + 500;
        continue;
      }
      const factor = diff.speed / Math.hypot(b.vx, b.vy);
      b.vx *= factor;
      b.vy *= factor;
      for (const segment of travel(b, 1 / 60)) {
        const p = (t: number) => ({
          x: old.x + (this.player.x - old.x) * t,
          y: old.y + (this.player.y - old.y) * t,
        });
        if (
          swept(
            p(segment.start),
          ...
          this.end = "dodge_collision";
      }
    }
    this.targets = this.targets.filter(
      (t) => !t.judged || this.time - t.at < 1000,
    );
    this.feedback = this.feedback.filter((f) => this.time - f.time < 700);
    if (!this.end && this.time >= this.chart.durationMs - 1e-7) {
      this.time = this.chart.durationMs;
      this.end = "chart_complete";
    }
  }
}
