...
브라우저에서 http://127.0.0.1:5173 을 엽니다. 시작·일시정지 화면의 **PC 이동 키**에서 선택한 WASD(기본)/방향키 또는 외곽 D-pad로
움직이고, 다른 손가락이나 마우스로 Match 원을 누릅니다. 링이 닿을 때 입력하며
모든 원을 누르되 캐릭터가 밟은 칸과 같은 색이면 콤보 적용 후 2배 점수를 받습니다. Escape 또는 우측 상단 버튼으로
일시정지합니다. 선택하지 않은 PC 키 묶음은 이동에 사용하지 않으며 설정은 기기에 저장됩니다. 세로 화면에서는 가로 전환 안내가 표시됩니다.
...
240초 First Orbit 차트, 247개 원, 고정 스텝 회피·벽 반사·충돌, 7색 판정,
처음부터 16칸·색 일치 성공 칸 즉시 재추첨, 점수·생명·콤보, 좌우 교환, 일시정지, 로컬 기록이 구현되어 있습니다.
음악은 120 BPM 테스트 클릭이며 최종 곡이 아닙니다. 사용자가 지정한 얼굴 무늬를 반영한 기본 v3 강아지와 로딩 자산을 사용하고,
색 일치 성공에는 로딩의 점프 포즈를, 야구공 충돌에는 기존 울음 포즈를 잠깐 표시합니다. 온라인 리더보드는 후속 범위입니다. 기록은 현재 브라우저에만 저장됩니다.
실제 iOS/Android 기기 성능과 터치 감각은 별도 플레이 테스트가 필요합니다.

난이도 v2: 야구공 9→96개(기존 3배), 속도 23.4→84.5U/s(30% 증가), 캐릭터 63U/s(30% 감소). 규칙 v3 이론상 상한 109,550점. 이전 버전 기록은 보존되지만 현재 순위와 분리됩니다.

현재 색 일치 규칙과 검증은 [v3 검증](docs/verification/tile-match-v3/results.md)을 참고하세요. 칸 색은 시간이 지나도 자동 변경되지 않으며, 성공한 칸만 공백 없이 새 색으로 바뀝니다.
